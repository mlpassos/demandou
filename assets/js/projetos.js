             ...
             var button = $(event.relatedTarget); // Button that triggered the modal
              ...
              $.ajax({
                ...
                success: function(data) {
                  ...
                  if (data.length==0) {
                    // nada
                    $('.modal-tarefas-lista').append('<p>Relaxe, sem tarefas no projeto pra você ainda.</p>');
                  } else {
                    data.forEach(function(item){
                      $('.modal-tarefas-lista').append('<p>Titulo:' +  item.titulo + '</p>'); 
                      $('.modal-tarefas-lista').append('<p>Início:' +  formataData(new Date(item.data_inicio)) + '</p>'); 
                      $('.modal-tarefas-lista').append('<p>Prazo:' +  formataData(new Date(item.data_prazo)) + '</p>'); 
                    });
                  }
                },
